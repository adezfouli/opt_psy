...
#################################################################################################
# this part is not used in the paper, but can be used in presentations. It proives some examples of reward fields
x1 = 0
x2 = 0
A1_ = function(x1, x2){
    H = 20
...
#################################################################################################
# Graphs for the effect of experimenal parameters.

# effect of response cost shown in Figure 4 in the paper.
T = 50
k = 1
l = 1
a = 1
...
# effect of ratio requirement. Figure 5 left-panel
T = 50
k = 1
l = 1
a = 0.3
...
# effect of deprivation level. Figure 5 middle panel.
T = 50
k = 1
l = 1
a = 0.3
...
# reinforcer magnitude. Figure 5 right panel
T = 50
k = 1
l = 1
a = 0.1
...
#################################################################################################
# for drawing cost and reward field for Figure 1 in the paper.
a = 0.02 * 16
b = 0
k = 4
dv = 0.01
...
#################################################################################################
# effect of session duration for Figure 2 in the paper.
k = 15
l = 1.
a = 0.05
b = 0.1
...
#################################################################################################
# for drawing distribution - Figure 3 in the paper.
ks = 0.7
sigma = 60 * (1 - ks)
t0 = 0
...
#################################################################################################
# effect of independent and dependent costs. Figure 6 in the paper left panel.
k = 1
l = 2
a = 1
b = NA
...
# rotational fields. Figure 6 in the paper right-panel.
# note that equations for alpha, v0, etc are extracted from ``rotational_solution.m'' file.
...
v0 = (H *
    ...
    (l - 1)) / (l * m - m +
    2 * m * tan((T * (l - 1)) / (2 * m)) +
    m * tan((T * (l - 1)) / (2 * m)) ^ 2 - l * m * tan((T * (l - 1)) / (2 * m)) ^ 2 + 2 * l * m * tan((T * (l - 1)) / (2 * m)))
x0 = (H * l + 2 * H * tan((T * (l - 1)) / (2 * m)) - H * l * tan((T * (l - 1)) / (2 * m)) ^ 2) / (l +
    2 * tan((T * (l - 1)) / (2 * m)) +
    tan((T * (l - 1)) / (2 * m)) ^ 2 +
    2 * l * tan((T * (l - 1)) / (2 * m)) -
    l * tan((T * (l - 1)) / (2 * m)) ^ 2 -
    1)
y0 = (H * (tan((T * (l - 1)) / (2 * m)) ^ 2 + 2 * l * tan((T * (l - 1)) / (2 * m)) - 1)) / (l +
    2 * tan((T * (l - 1)) / (2 * m)) +
    tan((T * (l - 1)) / (2 * m)) ^ 2 +
    2 * l * tan((T * (l - 1)) / (2 * m)) -
    l * tan((T * (l - 1)) / (2 * m)) ^ 2 -
